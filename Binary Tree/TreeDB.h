...
private:
   ...
   TreeNode* search (TreeNode* dbNode, string name, int& probesCount); 
   DBentry* place (TreeNode* dbNode, DBentry* newEntry);
   bool erase (TreeNode* dbNode, string name);
   TreeNode* maxNode (TreeNode* dbNode);
   bool isLeaf (TreeNode* dbNode); 
   bool isKey (TreeNode* dbNode, string key); 
   bool hasLNode (TreeNode* dbNode);
   bool hasRNode (TreeNode* dbNode);
   
   void empty (TreeNode* dbNode); 
   int count (TreeNode* dbNode) const; 
   void printTree (ostream& out, const TreeNode* dbNode) const;
